import Link from "next/link";

export default function AccessibilityStatementPagePt() {
  return (
    <main className="min-h-screen bg-white text-slate-900">
      <div className="mx-auto max-w-3xl px-6 py-16">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Legal</p>
        <h1 className="mt-3 text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">
          Declaração de acessibilidade
        </h1>
        <p className="mt-4 text-sm text-slate-600">
          Queremos que o DeepClinic Copilot seja acessível ao maior número possível de profissionais de saúde. Estamos a
          trabalhar para cumprir as principais diretrizes de acessibilidade web e iremos descrever aqui os nossos
          compromissos concretos.
        </p>
        <p className="mt-3 text-sm text-slate-600">
          Se encontrar barreiras de acessibilidade neste site ou nos nossos produtos, agradecemos que nos avise para que
          possamos corrigi‑las.
        </p>
        <div className="mt-8 flex flex-wrap items-center gap-4">
          <Link
            href="/pt/contact"
            className="inline-flex items-center justify-center rounded-full bg-[#d5ff7c] px-5 py-2 text-[13px] font-semibold text-slate-900 shadow-sm hover:bg-[#c9f46c]"
          >
            Enviar comentários de acessibilidade
            <span className="ml-1" aria-hidden>
              →
            </span>
          </Link>
          <Link href="/pt" className="text-sm font-medium text-slate-700 hover:text-slate-900">
            Voltar à página inicial
          </Link>
        </div>
      </div>
    </main>
  );
}
