...
export default function RecoveryPagePt() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error) {
        setFormError(error.message ?? "Não foi possível enviar o link. Tente novamente.");
        return;
      }

      setFormSuccess("Se o e-mail existir, enviamos instruções de recuperação.");
    } catch (error) {
      setFormError(error instanceof Error ? error.message : "Não foi possível enviar o link. Tente novamente.");
    } finally {
